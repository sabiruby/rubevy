...
| [plans/multi-vm-plan.md](plans/multi-vm-plan.md) | (Japanese) more than one VM in one app: what is already per-VM, the six places that assume one, three designs (type marker recommended), the stages, what the author decides first |
...
| [worklog/2026-09-17-multi-vm.md](worklog/2026-09-17-multi-vm.md) | a name tag on the VM, and then a second VM behind it (stages 0 and 1 of the multi-VM plan): why a default type parameter is filled in where a type is written but not where a value is, what that cost `RubevySet` (an enum no longer), which derives really add bounds to a generic, the component hook that took `::<M>` after all, and the two tests that failed for the right reasons — a script restarted by the very removal meant to end it, and a guard whose absence nothing could see until an asset was added between the two plugins |
