...
| [worklog/2026-09-17-multi-vm.md](worklog/2026-09-17-multi-vm.md) | a name tag on the VM (stage 0 of the multi-VM plan): why a default type parameter is filled in where a type is written but not where a value is, what that cost `RubevySet` (an enum no longer), which derives really add bounds to a generic, and the component hook that took `::<M>` after all |
