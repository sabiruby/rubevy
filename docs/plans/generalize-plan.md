...
| R9 | **済み**（2026-09-20、`generalize` の `54437b3` と `a0b7c02`、main に取り込み `57774f3`）。**拒まれた書き**: `Rubevy.rejected_writes`（Ruby、そのスクリプトが出した書きだけ。`[{entity:, name:, reason:}]`）と `ScriptWorld::rejected_writes() -> &[RejectedWrite]`（Rust、全部）。`warn!` は残した。覚えるのは 1 フレームぶんだが、**「直前のフレーム」ではなく「直前に書きのあったフレーム」**にした — スクリプトは「次のフレーム」に起きられない（`sleep 0` は VM の時計が次に進むまでで、時計は 4 ms 単位）ので、毎フレーム空にすると書いた本人が見る前に消える（その版を先に作ってテストが全滅して分かった）。上限は置かない: 拒まれた書き 1 件 33 命令、既定の予算で 1 フレーム約 6,000 件が天井で、予算が実際に縛ることを確かめた。**カメラ層**: `rubevy::layers::CAMERA` を `ScriptWorld::load_and_run` で読む（読まない app には `Rubevy::Camera` が無い）。`find` / `find_all` / `attach`、`position` / `move_to` / `pan`、`zoom 比`（2 倍に寄る。2D は `scale` を 1/2、3D は `tan(fov/2)` を 1/2）、`follow` / `unfollow`（相手が消えたら止まる）、`world_at x, y`（キューを返す。約束は `Rubevy.ask("camera.world_at", camera_entity, x, y)` — カメラは複数ありうるのでエンティティを第 1 引数に足した）、`reload`、`rejected_writes`。倍率は Ruby 側に持って絶対値で書くので同じ tick の 2 回の `zoom` が両方効く。Rust の依存 0 増、`src/` にカメラを知る Rust 無し、新しい数 0。171 passed。未実施: 拒まれなかったフレームの費用の前後計測（機械が塞がっていた。VM の命令数は増えていない）— `scale.md` の取り直しと一緒に本体がやる。記録は `docs/worklog/2026-09-20-rejected-writes.md`、`2026-09-20-camera-layer.md` |
| R11 | **済み**（2026-09-20、`generalize` の `458dfe9`・`0153a53`・`6ca5475`、main に取り込み `378c539`）。`Rubevy.next_frame`（次のフレームの tick の頭 — `$rubevy` の更新後・最初の VM の実行前 — に起き、`$rubevy[:frame]` と同じ番号を返す）と `Rubevy.each_frame { |dt, f| … }`。道は 3 つ比べて予約 kind `frame.next` を採った（答えずに待ち手の列へ移し、次の tick の頭で全員にフレーム番号を push。既存の同期読みと同じ道具だけ、VM は無変更。`Task#suspend` / `resume` の道は待ち手をホストが知る口が無く値も運べない、共有 `Task::Queue` は 1 件を 1 人しか取れない）。一時停止中のフレームは待ち抜けない。`frame_time` が足りなければ残りは列の先頭に残り、次のフレームでいちばん先に起きる。`next_frame` 1 回 = **62 命令**（`sleep 0` は 14）。1000 本が毎フレーム起きると 56,000 命令 = 既定の予算の 28%（時間は隣の担当が機械を使っている間の値で 2.4〜2.5 ms。静かな機械では未測定）。カメラ層の `follow` は `sleep` のまま（毎フレーム走るものに 62 命令 + 往復は高い。約束が要る人は自分のタスクに `each_frame`）。宿題: 計測器は既定値を `ScriptWorld` から読む、深さで切られた書きは `deeper than max_depth (2), so nothing under it was read or written` と言う、`host-api.md` に「Choosing `budget` and `frame_time` for your game」。**既定値は 1 つも動かしていない**。185 passed。記録は `docs/worklog/2026-09-20-next-frame.md`、`2026-09-20-r10-followups.md` |
| R10 | **済み**（2026-09-20、`generalize` の `a84394b` と `2f8f095`、main に取り込み `06a9d32`）。`docs/numbers.md`: **44 件**（`const` 33 行・数値リテラル 361 行から）。分類は (a) 不変量 8 / (b) 既に設定できる 14 / (c) 設定できるようにした 1 / (d) VM の数 3 / (e) 計測器 18。出どころは **測った 0** / 導出 7 / 引用 23 / 理由のみ 7 / **不明 7**。**rubevy の既定値は 1 つも測って決められていなかった**（測ってあるのは既定値が「買えるもの」のほう）。不明の 7: `budget` 200,000、`frame_time` 8 ms、`overrun` 50 ms、`max_depth` 16、ロードパスの `scripts` という名前、計測器の `mem` モードの既定、`ENTITY_TAG` の 1（`git log -S` で入ったコミットまで辿ったが、どれも数を述べるだけで理由が無い）。設定できるようにしたのは `reflect::MAX_DEPTH` → `ScriptWorld::max_depth()` / `set_max_depth()`（`pub` フィールドにしなかったのは、書きを VM から読み出すのがネイティブで `ScriptWorld` が見えず、フィールドにすると数が 2 か所になるため）。`queue_limit` 64 は著者判断どおり動かさず、rustdoc に「導出ではない・測った上と下・ゲームに合わせて選ぶ」。宿題: `magnification` に改名、`following?` が終わった後も真を返す件を直した（**計画書の前提違い**: カメラが消えても `move_to` は拒まれ続けない — 手を入れる前に測って分かった）、層は 2 回読んでも 1 回（`load_and_run` の戻り値を `Result<bool, String>` に。未公開の口なので本体は認めた）、文書 7 か所、`docs/README.md` に「Working on this repository」。178 passed。**既定値を動かす提案は下の行、著者判断待ち**。記録は `docs/worklog/2026-09-20-numbers-inventory.md`、`2026-09-20-layer-followups.md` |
| 既定値（R10） | **著者判断済み（2026-09-20）: 1 つも動かさない**。出どころ不明の 4 つ（`budget`・`frame_time`・`overrun`・`max_depth`）は `numbers.md` と rustdoc に「不明」と書いたまま。担当が見つけた一致 —「`frame_time` 8 ms ＝ 60 Hz の半分」「`overrun` 50 ms ＝ 60 Hz の 3 フレーム」— は記録ではなく観察なので**出どころとしては書かず**、`host-api.md` に「**選び方**」として書く: 自分のゲームの命令/ms を `FrameStats` で測り（rubevy の計測器は約 32,800 命令/ms で予算が先に効く。箱庭の**世界の VM** は約 9,300 命令/ms・予算 45,000 ≒ 4.8 ms でやはり予算が先。時計が先になるのは、同じ率で既定の予算 200,000 を計算した場合 ≒ 21 ms — **本体の最初の要約は逆に書いていた。R11 の担当が引用元を確かめて直した**）、目標フレーム時間のうちスクリプトに渡す分を VM の本数で割って `frame_time` にし、`budget` をそれに合わせる。R11 で `host-api.md` の「Choosing `budget` and `frame_time` for your game」に書いた |
| `scale.md` | 本体が静かな機械（idle 99%）で取り直して更新済み（`45f8069`）: 1000 台 `generous` 6.03 ms、3000 台 34.6 ms（R5 の担当の最初の表は隣の docker の最中のもので 16.8 ms / 83 ms だった）。天井は 3,900 件/フレーム |
...
| 09-21 S5b-2 | （games の S5b-2 から）**`ScriptWorld::budget` はタスクごとの持ち分を持たない**（`tick_scripts` は `task_run_limits` に残り全部を渡し、時分割は VM のスケジューラの中）。「1 タスクが 1 フレームに使ってよい量」をホストが言う口が無い。もう 1 つ: `Arg::Value` が `nil` を運び `Request::num` が `None` を返すので、番兵の値（Battle の `UNSET = -999.0`）は全部消せる — 文書に使いどころとして書く価値がある | `src/lib.rs` の `tick_scripts`、`docs/host-api.md`「What a question may carry」 | rubevy（API／文書） | 持ち分の口は Factory の F3（スクリプト付きの機械が数百台）で要るかが分かるまで足さない。`nil` の件は次に文書を触るときに 1 段落 |
| 09-21 S8 | （games の S8 から）**`host-api.md` の「Pausing」が、書きの着地について何も言っていない**。(1) 止まったフレームでも `RubevySet::Tick` の 4 本は走る — `tick_scripts` は 1 命令も走らせないが `drain_commands` / `apply_component_writes` / `apply_resource_writes` は走るので、前のフレームの書きが残っていれば止まりの中で着地する。(2) **`budget = 0` と「set に run condition を付ける」は同じではない** — 箱庭は world の VM を `RubevySet::<World>::tick().run_if(…)` で止めていて、set を止めるとその VM の書きの着地も止まる。(3) `start_scripts` は `Deliver` にあって `budget` を見ないので、止まっていても新しい `Script` はタスクになる（走りはしない）。どれもバグではなく、約束が文書から読めない | `docs/host-api.md`「Pausing」「Two VMs in one app」 | rubevy（文書と実物のずれ） | 計画に足す: 次に rubevy を触る段階（R7 の rubevy 側か、`frame_time` の下限の件と一緒）で 3 点を書く。S8 は R4 を疑って始まったが、**R4 は無関係だった**（前後を交互に 20+20 走行して同じ形が両方に出た） |
| 09-20 R11 | **`frame_time` を極端に小さくすると VM が 1 命令も走らない**（答えループの頭の時間判定で抜けるので「遅れていても 1 件は答える」に届かない）。`frame_time = 1µs` と書いたゲームはスクリプトが完全に止まる | `src/lib.rs` の `tick_scripts` | rubevy（口の設計） | **本体が原則から決めた**: 下限の数は置かない（出どころが無い）。「必ず 1 周は走らせる」形にできるかを次に rubevy を触る段階で見て、できなければ rustdoc に 1 文 |
| 09-20 R11 | 待ちタスク 1 本あたり約 2.5 µs は VM の側（起こすとき待ち行列を歩く）。`each_frame` が広まると「1000 体が載らない」の主因になる | sabiruby のスケジューラ | VM | sabiruby の計画 H1 の根拠に足す（`each_frame` 1000 本の行を H1 の計測に入れる） |
| 09-20 R11 | 箱庭の `Rubevy.ask("frame")`（ゲームが毎フレーム答える自作の待ち）は `next_frame` に置き換えられる。ただし箱庭の `each_frame` は being ごとの呼び出しも兼ねていて 1 対 1 ではない | rubevy_games `garden` | ゲーム固有 | games の計画 7 章に写す（S5b のついでか別に） |
| 09-20 R11 | 読みの側には「深すぎた」と「本当に nil」の区別が無い。`host-api.md` の「Time」の表に `queue_limit` と `max_depth` が無い。計測器の `mem` モードの既定に rustdoc が無い | `src/reflect.rs`、`docs/host-api.md`、`examples/how_many_scripts.rs` | rubevy（小） | 見送り（次に文書を触るときに） |
| 09-20 R10 | **計測器 `Limits::Default` が既定値を写している**（`scripts.budget = 200_000; frame_time = 8 ms` と直書き、名札が 3 度目の写し）。既定値が動いた日に `default(200k/8ms)` の行が既定でない設定を測る。「Every one of them is an argument」も正確でない（台数と sleep は一覧から選べるだけ、購読者の数などは `const`） | `examples/how_many_scripts.rs:272-283`、`how_many_subscribers.rs` | rubevy（計測器） | 計画に足す: R11 のついでに、既定は `ScriptWorld` から読む・名札もそこから作る・一覧に無い値も引数で渡せる、に直す |
| 09-20 R10 | 深さを超えた書きの断り文句が深さの話をしない（境界の 1 つ先は鍵も値も nil で届くので `a field name must be a Symbol or a String` と言う）。消えたエンティティへの書きは黙って何も起きず、`rejected_writes` にも出ない | `src/reflect.rs`、`apply_component_writes` | rubevy（表示／口の設計） | 断り文句は R11 のついでに直す。消えた相手は見送り（欲しい利用者が出てから） |
| 09-20 R9 | **`initialize` の中ではワールドを読めない**。`Class#new` がネイティブなので `initialize` 内の `e[:Transform]` は `blocking pop cannot be called from within a C function boundary` で落ち、スクリプトは黙って Failed で終わる（ブロック・`times`・`while`・普通のメソッドは全部通る。7 通り比べて確かめた）。外の利用者が必ず踏む | `docs/host-api.md` の「Why a read can wait inside `[]`」はブロックの話だけ | rubevy（文書）／sabiruby | 計画に足す: R10 で文書に 1 段落。VM 側（`new` の境界）は sabiruby の計画の 7 章に写す |
| 09-20 R9 | **スクリプトから「次の 1 フレームだけ待つ」と言えない**（`sleep 0` は「時計が次に進むまで」、時計は 4 ms 単位）。`host-api.md` の「Time」に 4 ms の粒度が書かれていない | rubevy の口 | rubevy（口の設計） | **本体が原則から決めた: 足す**（R11）。フレームはゲームの基本の単位で、外の利用者が自作せずに言えるべき。文書の 4 ms は R10 |
| 09-20 R9 | **`Rubevy::Camera#scale`（倍率、大きいほど寄る）と bevy の `Projection::scale`（小さいほど寄る）が同じ名前で逆の意味**（example の出力に `4.0x` と `scale 0.25` が並ぶ）。計画書（本体）の語がそうだった | `src/layers/camera.rb` | rubevy（層の命名） | **本体が原則から決めた: `magnification` に改名**（R10 で。未公開なので壊れる利用者はいない）。同じ語で逆の意味は外の利用者への罠。著者が別の語を望めば 1 行で替わる |
| 09-20 R9 | `follow` の子タスクはスクリプト本体が終わっても走り続け、カメラ自身が despawn されると `move_to` が毎フレーム拒まれ続ける。`Rubevy.rejected_writes` は「どの行が書いたか」を持たない。層を 2 枚目以降どう足すかの決まりが無い（同じ層を 2 回読むと 2 回走る） | `src/layers/camera.rb`、`src/lib.rs` | rubevy（子タスクの寿命／層の決まり） | 計画に足す: R10 で、`follow` はカメラが消えたら止まる・層は 2 回読んでも 1 回、を直す。行番号は使う人が出てから |
