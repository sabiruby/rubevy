...
| R11 | 実行中。**スクリプトが「次の 1 フレームだけ待つ」と言える口**（R9 が見つけた不足。下の 7 章）。箱庭は `Rubevy.ask("frame")` にゲームが毎フレーム答える形を自作している — 外の利用者も同じものを書くことになる |
