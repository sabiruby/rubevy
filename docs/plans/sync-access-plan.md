...
| S3 | **済み**（rubevy_games `a490d55`、2026-09-17）。順序 `.before(RubevySet::Tick)`、命令数/判断、パネルの `Waiting::Read`、docs。selftest 6 番は「0.5 秒の窓を毎フレーム見る」に測り方を直して 10/10（242 接触、外れ 0。閾値は不変）。**残る揺れ**: 4 番（すり抜け）は順序と無関係に 1/24 → 1/26、5 番（空腹の個体が草に着く）は順序を入れてから 3/26（前は 0/24、p ≈ 0.24 で断定できず。通るときは毎回 1.78 秒、落ちるときは 2.0〜2.2 で止まる）。どちらも未着手。世界の脚本の作業（`garden-world-plan.md` の書き直し）で規則が動くときに一緒に見る |
| S4 | **済み**（`5f112f7` `eba7288`、2026-09-17）。`answer_in_tick`、tests 73 件、`examples/nearest.rs`（閉包 0 フレーム / システム 1 フレームの対比）、unsafe 0。集合をまたぐ順は「rubevy の読みが先、ゲームの閉包が後」。予約 4 種を登録しても静かに無視される（docs に記載。`warn!` は未）。閉包 1 回の費用は未計測（W1 で `garden.within` を測る） |
| S5 | 後で判断 |
